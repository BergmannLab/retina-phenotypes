...
num_ventile = sys.argv[1] #2
input_dir = sys.argv[2] #'/NVME/decrypted/scratch/multitrait/UK_BIOBANK_ZERO/gwas/2022_08_03_ventile'+str(num_ventile)+'/'
save_results = sys.argv[3] # '/NVME/decrypted/scratch/multitrait/UK_BIOBANK_ZERO/genes/'
phenotypes_type=sys.argv[4] #'main'
...
if phenotypes_type=='complementary':
    filenames= list(sys.argv[7].split(","))
    filenames_new= list(sys.argv[8].split(","))
    size_a=35
    size_b=35
elif phenotypes_type=='main':
    filenames= list(sys.argv[5].split(","))
    filenames_new= list(sys.argv[6].split(","))
    size_a=12
    size_b=12
...
f_g.plot(save_results, csv_name, DATE, num_ventile, size_a, size_b)
