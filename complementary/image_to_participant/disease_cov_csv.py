...
list_interest= ['assessment_centre', 'ethnic_background', 'date_center', 'age_center', 'DBP', 'SBP',  'PR', 'age_current_smoker', 'alcohol_intake_frequency',
...
for disease_name in list_interest:
    ...
    df_diseases_cov[disease_name_aux] = np.where(df_diseases_cov["instance"] == 0, df_diseases_cov[disease_name_00], df_diseases_cov[disease_name_10])
    df_diseases_cov[disease_name_both] = np.where(df_diseases_cov["instance"] == np.nan, np.nan, df_diseases_cov[disease_name_aux])
