...
# OPTION 0
rm -f $dir_ARIA_output/* # clear old ARIA output
cd $script_dir
if (( $batch_max > 0 )); then
    for i in $(seq 1 $step_size $batch_max); do ($matlab_dir -nodisplay -nosplash -nodesktop -r "ARIA_run_tests 0 REVIEW $dir_input $classification_output_dir $TYPE_OF_VESSEL_OF_INTEREST 0.79 $script_dir $i $step_size -1 999999 -1 999999 $dir_ARIA_output $aria_processor" > $helper_dir/batch$i.txt &); done
fi
$matlab_dir -nodisplay -nosplash -nodesktop -r "ARIA_run_tests 0 REVIEW $dir_input $classification_output_dir $TYPE_OF_VESSEL_OF_INTEREST 0.79 $script_dir $(($batch_max + 1)) $remainder -1 999999 -1 999999 $dir_ARIA_output $aria_processor" > $helper_dir/batch$(($batch_max + 1 )).txt &
